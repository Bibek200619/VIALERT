...
interface GraphMapProps {
  ...
  previousRoutePath?: [number, number][];
  roadPaths?: Record<string, [number, number][]>;
  theme?: 'default' | 'light';
}
...
export function GraphMap({
  ...
  const routePoints = (routePath ?? routeNodes.map((node) => [node.lat, node.lng] as [number, number]))
    .map(([lat, lng]) => project(city, lat, lng).join(','))
    .join(' ');
  const previousRoutePoints = (previousRoutePath ?? previousRouteNodeIds.flatMap((id) => {
  ...
  return <div className={`graph-map${light ? ' light-map' : ''}${followAmbulance ? ' follow-camera' : ''}${routeTone === 'standard' ? ' standard-route' : ''}`} role="img" aria-label={`Bengaluru demo road graph. Route from ${base?.name ?? 'base'} to ${destinationName}; simulated vehicles, signals, and incident overlays are marked.`}>
    <svg viewBox={viewBox} aria-hidden="true" focusable="false">
      <defs>
        <pattern id="map-grid" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke={light ? '#dce4e9' : '#26352e'} strokeWidth="1" /></pattern>
      ...
      <rect width="840" height="480" fill={light ? '#f1f4f5' : '#111a17'} />
      <rect width="840" height="480" fill="url(#map-grid)" />
      <path d="M0 400 C180 350 170 230 300 205 S510 260 640 175 730 90 840 65" fill="none" stroke={light ? '#e2e8eb' : '#1e2c26'} strokeWidth="56" />
      <path d="M0 400 C180 350 170 230 300 205 S510 260 640 175 730 90 840 65" fill="none" stroke={light ? '#cad5da' : '#25342d'} strokeWidth="2" strokeDasharray="9 12" />
      {city.roads.map((road) => {
        const from = nodeById.get(road.from);
        const to = nodeById.get(road.to);
        if (!from || !to) return null;
        const points = (roadPaths?.[road.id] ?? [[from.lat, from.lng], [to.lat, to.lng]])
          .map(([lat, lng]) => project(city, lat, lng).join(','))
          .join(' ');
        return <polyline key={road.id} points={points} className={`graph-road ${road.blocked ? 'blocked' : ''}`} />;
      })}
