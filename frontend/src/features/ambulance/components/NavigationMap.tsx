...
interface NavigationMapProps {
  ...
  simulationFollowing?: boolean;
}

function markerIcon(kind: 'ambulance' | 'base' | 'hospital' | 'signal' | 'warning', label: string, simulationFollowing = false) {
  return L.divIcon({
    className: `vialert-marker-host ${kind}${simulationFollowing && kind === 'ambulance' ? ' simulation-following' : ''}`,
...
export function NavigationMap({ city, route, ambulance, destinationName, previousRouteNodeIds = [], navigationMode = false, simulationFollowing = false }: NavigationMapProps) {
  ...
  return <section className={`navigation-map-panel ambulance-driver-map${navigationMode ? ' is-navigation' : ''}${simulationFollowing ? ' simulation-following' : ''}`} aria-label="Bengaluru emergency navigation map">
    <div className="map-frame">
      {showGraph ? <GraphMap city={city} route={route} ambulance={ambulance} destinationName={destinationName} previousRouteNodeIds={previousRouteNodeIds} /> : <MapContainer center={center} zoom={12} scrollWheelZoom className="leaflet-map" zoomControl={false}>
        ...
        {ambulance && <Marker position={[ambulance.lat, ambulance.lng]} icon={markerIcon('ambulance', '▲', simulationFollowing)} title="Ambulance 07 simulated location" alt="Ambulance 07 simulated location"><Popup>Ambulance 07 · simulated movement</Popup></Marker>}
