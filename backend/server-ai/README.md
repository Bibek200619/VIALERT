...
```bash
uv sync --project backend/server-ai --locked
uv run --project backend/server-ai uvicorn app.main:app --app-dir backend/server-ai --reload --host 127.0.0.1 --port 8000
```

Open `http://127.0.0.1:8000/docs` for interactive API documentation. Run the checks with:

```bash
uv run --project backend/server-ai pytest backend/server-ai/tests
```
