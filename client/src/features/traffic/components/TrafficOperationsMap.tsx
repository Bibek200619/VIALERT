...
export function TrafficOperationsMap({ city, vehicles, selected, incidents, focusedNodeId, onClearLocation }: { city: CityData; vehicles: OperationsVehicle[]; selected: OperationsVehicle | null; incidents: IncidentRecord[]; focusedNodeId: string | null; onClearLocation: () => void }) {
  ...
  return <section className="panel traffic-map-panel" aria-labelledby="traffic-map-title">
    <div className="traffic-panel-heading traffic-map-heading"><div><span className="eyebrow">City overview · map</span><h2 id="traffic-map-title">Operations map</h2><p>Selected route, fleet positions, signals, and mock incident overlays.</p></div><span className="map-mode"><i aria-hidden="true" />{useGraph ? 'Shared graph' : 'Street layer'}</span></div>
