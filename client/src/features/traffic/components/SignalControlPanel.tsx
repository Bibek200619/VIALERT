...
export function SignalControlPanel({ city, signals, vehicle, onChange }: { city: CityData; signals: Signal[]; vehicle: OperationsVehicle | null; onChange: (id: string, payload: Partial<Pick<Signal, 'state' | 'mode'>>, confirmed?: boolean) => Promise<boolean> }) {
  ...
  return <section className="panel traffic-signal-panel" aria-labelledby="traffic-signal-title">
    ...
    {signals.length === 0 || !signal ? <p className="traffic-empty">No signals are available.</p> : <>
      <label className="traffic-filter-label" htmlFor="signal-select">Select monitored signal</label><select id="signal-select" value={signal.id} onChange={(event) => { setSelectedId(event.target.value); setConfirming(false); }}>{signals.map((item) => <option key={item.id} value={item.id}>{item.id} · {locationName(city, item.nodeId)}</option>)}</select>
