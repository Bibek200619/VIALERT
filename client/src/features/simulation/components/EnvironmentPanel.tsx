...
export function EnvironmentPanel({ city, state }: EnvironmentPanelProps) {
  const active = state.scenarios.filter((scenario) => scenario.active);
  ...
  return <section className="panel environment-panel" aria-labelledby="environment-panel-title">
    <div className="panel-heading-row"><div><span className="eyebrow">City conditions · mock</span><h2 id="environment-panel-title">Environment &amp; roads</h2></div><span className={`environment-state ${active.length ? 'warning' : ''}`}><i aria-hidden="true" />{active.length ? 'Scenario active' : 'Clear demo baseline'}</span></div>
    ...
    {active.length > 0
      ...
      : <p className="environment-empty">Activate a scenario to see its local weather or roadway effect here.</p>}
